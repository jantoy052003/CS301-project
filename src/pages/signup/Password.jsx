...
const Password = ({ value, onChange, onErrorChange }) => {
  ...
  const handlePasswordError = () => {
    if (value.trim() === "") {
        ...
        setPasswordError("Minimum of 8 characters is required")
    } else {
  ...
  return (
    <div className="mb-6 relative">
      ...
      />
      <label htmlFor="password" className="transition-all duration-300 text-orange-500 absolute top-3 left-4 translate-y-[-50%] text-[14px] lg:text-[14px] peer-placeholder-shown:text-base peer-placeholder-shown:top-1/2 peer-placeholder-shown:text-red peer-focus:text-orange-400 peer-focus:absolute peer-focus:top-3 peer-focus:left-4 peer-focus:translate-y-[-50%] peer-focus:text-[14px]">
        Password
      </label>
      {passwordError && (
        <>
